#!/usr/bin/env python3
"""
Basin Battery Lifecycle Monitor
================================
Standalone ROS2 monitor script that subscribes to Basin's topics and writes
metrics to InfluxDB for Grafana visualization. Runs natively on the laptop.

Usage:
    python3 basin_monitor.py

Requires: rclpy (ROS2), influxdb-client, paramiko
"""
...
# paramiko removed — using subprocess ssh instead

# ---------------------------------------------------------------------------
# Configuration from environment
# ---------------------------------------------------------------------------
BASIN_IP = os.environ.get("BASIN_IP", "192.168.1.158")
BASIN_SSH_USER = os.environ.get("BASIN_SSH_USER", "ubuntu")
BASIN_SSH_PASSWORD = os.environ.get("BASIN_SSH_PASSWORD", "turtlebot4")
BASIN_NS = os.environ.get("BASIN_NAMESPACE", "/Basin").strip("/")
...
INFLUXDB_BUCKET = os.environ.get("INFLUXDB_BUCKET", "basin")
...
# Main monitor node
# ---------------------------------------------------------------------------
class BasinMonitor(Node):

    def __init__(self):
        super().__init__("basin_monitor")
        self.get_logger().info(f"Basin Monitor starting — NS=/{BASIN_NS}, IP={BASIN_IP}")
        ...
        self._ssh_cmd_prefix = [
            "sshpass", f"-p{BASIN_SSH_PASSWORD}",
            "ssh",
            "-o", "PubkeyAuthentication=no",
            "-o", "StrictHostKeyChecking=no",
            "-o", "ConnectTimeout=5",
            f"{BASIN_SSH_USER}@{BASIN_IP}",
        ]
    ...
    def _check_network(self):
        """Ping Basin and collect WiFi signal, write topic rates."""
        ping_ms = self._ping_basin()
    ...
    def _ping_basin(self) -> float | None:
        try:
            result = subprocess.run(
                ["ping", "-c", "1", "-W", "2", BASIN_IP],
                capture_output=True,
                text=True,
                timeout=3,
            )
    ...
    def _poll_system(self):
        """SSH to Basin Pi for CPU temp, RAM, load average."""
        try:
            cmd = (
    ...
    def shutdown(self):
        ...
        try:
            ...
            pass
        self.get_logger().info("Basin Monitor shutdown complete")
